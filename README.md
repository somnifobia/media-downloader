...
For X / Twitter support: yt-dlp available in your PATH.
...
internal/app contains a router that parses the URL and chooses the correct downloader based on the host.

internal/ytdl uses github.com/kkdai/youtube/v2 to fetch video metadata and formats, prefers muxed MP4 formats and streams the response to disk while updating a progress bar from github.com/schollz/progressbar/v3.

internal/twitterdl extracts the tweet ID from the URL and calls the external yt-dlp binary via os/exec with a predefined output pattern.
...
Add support for more platforms by implementing native downloaders in Go or delegating to yt-dlp.
